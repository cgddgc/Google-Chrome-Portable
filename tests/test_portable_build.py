...
class BuildTargetTests(unittest.TestCase):
    def test_default_targets_include_only_chrome(self):
        targets = portable_build.get_default_targets()
        self.assertEqual([target.output_name for target in targets], ["Chrome"])
...
class BuildNameTests(unittest.TestCase):
    def test_build_release_name_contains_chrome_version(self):
        name = portable_build.build_release_name(
            chrome_version="135.0.7049.42",
            build_date=date(2026, 5, 23),
        )

        self.assertEqual(name, "Win64_chrome_135.0.7049.42_2026-05-23")
...
class MainFlowTests(unittest.TestCase):
    def test_main_extracts_nested_chrome_archive_and_writes_build_name(self):
        with TemporaryDirectory() as temp_dir:
            ...
            self.assertTrue(
                (temp_path / "build" / "release" / "Chrome" / "App" / "version.dll").exists()
            )
            self.assertIn(
                "BUILD_NAME=Win64_chrome_135.0.7049.42_2026-05-23",
