...
- GitHub Actions 每次会生成两个 artifact：`Chrome_<version>_win64` 与 `Chromium_<version>_<revision>_win64`，下载时分别对应两个 zip 文件。Chromium 的 `<revision>` 是最新 Win64 snapshot 的 commit position，`<version>` 会在解包后从 `chrome.exe` 读取。
