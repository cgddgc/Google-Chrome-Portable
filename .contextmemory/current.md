...
4. 让 Chromium artifact 名称包含真实产品版本号和 snapshot revision。
...
当前未提交改动包括：
- `portable_build.py`: 恢复双目标构建；新增 Chromium LAST_CHANGE 元数据获取、`chrome-win.zip` 下载模板、`chrome-win` 源目录解析；Chrome 与 Chromium 均输出到 `build/release/<Name>/App`；Chromium 解包后从 `App/chrome.exe` 读取产品版本；写出 `CHROME_ARTIFACT_NAME` / `CHROMIUM_ARTIFACT_NAME`。
- `tests/test_portable_build.py`: 更新为 19 个测试，覆盖 Chrome/Chromium 双目标、Chromium metadata、Chromium revision 数字校验、Windows VERSIONINFO 产品版本读取、`chrome-win` 目录解析、Chromium 扁平解压目录 fallback、双目标主流程、Chrome++ 配置覆盖、两个独立 artifact 名称。
- `.github/workflows/build.yml`: job 名称为 `Warp_Browsers`，分别上传 `build/release/Chrome` 和 `build/release/Chromium`，artifact 名称为 `Chrome_<version>_win64` 与 `Chromium_<version>_<revision>_win64`。
- `README.md`: 恢复 Chrome + Chromium 双产物说明，并说明 Chromium `<version>` 来自解包后的 `chrome.exe`，`<revision>` 来自最新 Win64 snapshot commit position。
...
已确认外部事实：
- Chromium Windows snapshot 最新入口继续使用 `https://storage.googleapis.com/chromium-browser-snapshots/Win_x64/LAST_CHANGE`。
- `LAST_CHANGE` 返回的是 snapshot revision / commit position，不是产品版本号。
...
- Chromium artifact 名称现在为 `Chromium_<product_version>_<revision>_win64`；`product_version` 来自解包后的 `Chromium/App/chrome.exe` Windows version resource。
